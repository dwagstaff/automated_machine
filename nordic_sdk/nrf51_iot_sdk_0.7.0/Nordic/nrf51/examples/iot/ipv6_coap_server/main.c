...
#define DEVICE_NAME                     "nCOAP_Server"                                        /**< Device name used in BLE undirected advertisement. */
...
void well_known_core_callback(coap_message_t * p_request, coap_message_t * p_response)
{
    ...
    err_code = coap_message_payload_set(p_response, well_known_core, \
                                        strlen((char *)well_known_core));
    APP_ERROR_CHECK(err_code);
}


static void lights_callback(coap_message_t * p_request, coap_message_t * p_response)
{
    // No implementation necessary.
}


static void handle_led_request(uint32_t led_mask,            \
...
static void led3_callback(coap_message_t * p_request, coap_message_t * p_response)
{
    handle_led_request(LED_THREE, &p_request, &p_response);
}


static void led4_callback(coap_message_t * p_request, coap_message_t * p_response)
{
    handle_led_request(LED_FOUR, &p_request, &p_response);
}
...
static void coap_endpoints_init(void)
{
    ...
    lights.callback = lights_callback;
