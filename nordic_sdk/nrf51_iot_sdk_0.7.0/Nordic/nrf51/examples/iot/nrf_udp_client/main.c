...
static void tx_timeout_handler(void * p_context)
{
    ...
    switch (m_node_state)
    {
        ...
        case APP_STATE_IPV6_IF_UP:
        {
            ...
            ipv6_addr_t link_local_ipv6_addr;
            eui64_t     local_eui64;
            memset(&local_eui64, 0, sizeof(eui64_t));
            memset(&link_local_ipv6_addr, 0, sizeof(ipv6_addr_t));

            IPV6_EUI64_CREATE_FROM_EUI48(local_eui64.identifier,
                                         m_local_ble_addr.addr,
                                         BLE_GAP_ADDR_TYPE_PUBLIC);

            IPV6_CREATE_LINK_LOCAL_FROM_EUI64(&link_local_ipv6_addr, &local_eui64.identifier[0]);
