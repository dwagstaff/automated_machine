...
void notify_disconnection(const mqtt_client_t * p_id)
{
    mqtt_evt_t    evt;
    mqtt_t * p_client = &mqtt_client[*p_id];

    if(p_client->state != MQTT_STATE_CONNECTED)
    {
...
static void tcp_close_connection(const mqtt_client_t * p_id)
{
    mqtt_t * p_client = &mqtt_client[*p_id];
    ...
    if (err == ERR_OK && p_buffer != NULL)
    {
        ...
        switch(payload[0] & 0xF0)
        {
            ...
            case MQTT_PKT_TYPE_PUBLISH:
            {
                ...
                {
                    // QoS diffrent from 0, Message Id present.
                    offset += 2;
                    packet[2] = payload[2];
                    packet[3] = payload[3];
                }
...
uint32_t mqtt_abort(const mqtt_client_t * p_client)
{
    ...
    if ((mqtt_client[index].state == MQTT_STATE_CONNECTED) ||
        (mqtt_client[index].state == MQTT_STATE_CONNECTING) ||
        (mqtt_client[index].state == MQTT_STATE_TCP_CONNECTED) ||
...
uint32_t mqtt_ping(const mqtt_client_t * p_id)
{
    ...
    //Ping
    err_t err = transport_write(p_client, (uint8_t *)m_ping_packet, 2);
    return err;
}
