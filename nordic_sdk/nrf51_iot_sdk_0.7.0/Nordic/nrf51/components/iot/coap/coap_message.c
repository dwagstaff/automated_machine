...
uint32_t coap_message_create(coap_message_t * p_message, coap_message_conf_t * p_init_config)
{
    ...
    p_message->arg               = p_init_config->arg;
                             ...
                             const coap_remote_t *  p_address)
{
    ...
    for (uint8_t index = 0; index < p_message->header.token_len; index++)
    {
    ...
    while ((p_raw_message[byte_index] != COAP_PAYLOAD_MARKER) && (byte_index < message_len))
    {
