...
/**
 * @brief Function for setting the output based on a pin mask.
 *
 * @param pin_mask The pins to set high.
 */
static __INLINE void nrf_gpio_out_set(uint32_t pin_mask)
{
	NRF_GPIO->OUTSET = pin_mask;
}

/**
 * @brief Function for clearing the output based on a pin mask.
 *
 * @param pin_mask The pins to clear.
 */
static __INLINE void nrf_gpio_out_clr(uint32_t pin_mask)
{
